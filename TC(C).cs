...
public class Macro : ReportMacroProvider
{
	...
	private string[] Filter_VO = new string[]{"ОР","ОШ","ПЧ","ОП","ОК","ОУ","ПЦ","Ш-","П-","1П","2П","3П",
		"4П","5П","6П","7П","8П","9П","КИ","СН","ПФ","ми","ри","1Р","2Р","3Р","4Р","5Р","6Р","7Р","1М","2М","3М","4М","5М","6М"};
	private int cntNomGlobal = 0;
	...
	{
		{
			...
			if (_allassemblyListsort.Any())
			{
				...
				if (DSE.Class.Name == "Сборочная единица")
				{
					...
					if (техпроцессы.Any() && ТП != null)
						Data_output(ТП, дочерний, ведомость, выводитьОперацию, текст, В_шаблон, Т_шаблон, Д_шаблон, С_шаблон, Рз_шаблон);
				}
				else
				{
					foreach (ReferenceObject child in _allassemblyListsort)
					{
						...
						Объект ТП = техпроцессы.FirstOrDefault(t => t["Вариант"].ToString() == "" &&
															   t.Тип.ПорожденОт("Базовый технологический процесс"));
						if (типдочернего == "Сборочная единица" && ТП != null && техпроцессы.Any())
							Data_output(ТП, дочерний, ведомость, выводитьОперацию, текст, В_шаблон, Т_шаблон, Д_шаблон, С_шаблон, Рз_шаблон);
					}
				}
			}
			...
			if (_alldetailListsort.Any())
			{
				...
				else
				{
					foreach (ReferenceObject child in _alldetailListsort)
					{
						...
						Объект ТП = техпроцессы.FirstOrDefault(t => t["Вариант"].ToString() == "" &&
															   t.Тип.ПорожденОт("Базовый технологический процесс"));
						if (типдочернего == "Деталь" && ТП != null && техпроцессы.Any())
						{
							Data_output(ТП, дочерний, ведомость, выводитьОперацию, текст, В_шаблон, Т_шаблон, Д_шаблон, С_шаблон, Рз_шаблон);
						}
					}
	...
	public void Карта(string карта)
	{
		...
		foreach (Инструкция инструкция in инструкцииТП.OrderBy(i => i["Текст инструкции"].ToString()))
		{
	...
	private string получитьВсеИнструкцииПоОперациям(ТехнологическийПроцесс техпроцесс)
	{
		...
		foreach (Операция операция in техпроцесс.Операции)
		{
			...
			{
				// указан шифр инструкции и его нет в списке
				if (ВыполнитьМакрос(Макрос_Теги, "инструкцияОТ", инструкцияОперации) && инструкцияОперации.Шифр != String.Empty && !инструкции.Contains(инструкцияОперации.Шифр))
				{
	...
	public void Data_output(Объект ТП, Объект дочерний, string ведомость, bool выводитьОперацию, Текст текст, ШаблонСтроки В_шаблон,
							ШаблонСтроки Т_шаблон, ШаблонСтроки Д_шаблон, ШаблонСтроки С_шаблон, ШаблонСтроки Рз_шаблон)
	{
		...
		if (koloper > 0)
		{
			...
			строкаС["Номер"].Текст = cntNomGlobal.ToString();

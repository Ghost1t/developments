...
{
    private List<NomenclatureObject> _productList = new List<NomenclatureObject>();
    private List<NomenclatureObject> _assemblyList = new List<NomenclatureObject>();
    private List<NomenclatureObject> _detailList = new List<NomenclatureObject>();
    private List<NomenclatureObject> _materialList = new List<NomenclatureObject>();
    private List<NomenclatureObject> _completeList = new List<NomenclatureObject>();
    private List<NomenclatureObject> _sortassemly1 = new List<NomenclatureObject>();
    private List<NomenclatureObject> _sortassemly2 = new List<NomenclatureObject>();
    private List<NomenclatureObject> _sortassemly3 = new List<NomenclatureObject>();
    private List<NomenclatureObject> _sortassemly4 = new List<NomenclatureObject>();
    private List<NomenclatureObject> _sortdetail1 = new List<NomenclatureObject>();
    private List<NomenclatureObject> _sortdetail2 = new List<NomenclatureObject>();
    private List<NomenclatureObject> _sortdetail3 = new List<NomenclatureObject>();
    private List<NomenclatureObject> _sortdetail4 = new List<NomenclatureObject>();
    private List<NomenclatureObject> _allassemblyListsort = new List<NomenclatureObject>();
    private List<NomenclatureObject> _alldetailListsort = new List<NomenclatureObject>();
    ...
    private const int ширинаДляГоризонтальнойФормы = 131;    //Ширина ячейки таблицы (в символах) для текста перехода в карте
    private const int ширинаДляГоризонтальнойФормыСВременем = 109;    //Ширина ячейки таблицы (в символах) для текста перехода в карте, где указывается время
    ...
    public void Карта_эскизов()
    {
        ...
        //Объект операц = ТекущийОбъект;

        //foreach (var опер in операц.ДочерниеОбъекты.Where(t => string.IsNullOrEmpty(t["Вариант"].ToString())))
        //{
        ...
        if (эскиз != null)
        {
            ...
            else
            {
                ...
                Строка страницаОтчёта = текст.Таблица.ДобавитьСтроку(Стр_шаблон);
                страницаОтчёта["Номер"].Текст = операция.Номер.ToString();
                страницаОтчёта["Страница"].ВставитьЭскиз(эскиз);
            }
        }
        else
        ...
        foreach (var пер in операция.ДочерниеОбъекты.Where(t => string.IsNullOrEmpty(t["Вариант"].ToString())))
        {
            Переход переход = (Переход)пер;

            //Обработка графической части
            Объект эскизПер = переход.Эскиз;
            ...
            if (эскизПер != null)
            {
                ...
                //- Эскиз CAD может вставляться как многостраничный, так и масштабированный в ячейку. 
                //- Изображение (jpg, bmp, png...) вставляется только в ячейку.*/
                if (эскизПер.Тип.ПорожденОт("Документ системы T-FLEX CAD"))
                {
                    //Получение количества страниц эскиза
                    int количествоСтраницПер = Эскиз.ПолучитьКоличествоСтраниц(эскизПер);
                    if (количествоСтраницПер > 1)
                    {
                        //Создание необходимого количества страниц для отображения многостраничного эскиза
                        for (int i = 1; i <= количествоСтраницПер; i++)
                        {
                            Строка страницаОтчётаПер = текст.Таблица.ДобавитьСтроку(Стр_шаблон);
                            страницаОтчётаПер["Номер"].Текст = "Переход №" + переход.Номер.ToString();
                            страницаОтчётаПер["Страница"].ВставитьЭскиз(эскизПер, i);
                        }
                    ...
                    else
                    {
                        ...
                        Строка страницаОтчётаПер = текст.Таблица.ДобавитьСтроку(Стр_шаблон);
                        страницаОтчётаПер["Номер"].Текст = "Переход №" + переход.Номер.ToString();
                        страницаОтчётаПер["Страница"].ВставитьЭскиз(эскизПер);
                    }
                }
                else
                {
    ...
    public void Карта_эскизов_ТП()
    {
        ...
        foreach (var опер2 in ТП.ДочерниеОбъекты.Where(t => string.IsNullOrEmpty(t["Вариант"].ToString())))
        {
            Операция операция = (Операция)опер2;
            ВыполнитьМакрос(Макрос_Теги, "ОчиститьЗамечания");
            ВыполнитьМакрос(Макрос_Переменные, "ЗаполнитьОсновныеНадписи", операция);
            ...
            foreach (var пер in опер2.ДочерниеОбъекты.Where(t => string.IsNullOrEmpty(t["Вариант"].ToString())))
            {
                Переход переход = (Переход)пер;
    ...
    {
        Объекты изготавливаемыеДСЕ = ТекущийОбъект.СвязанныеОбъекты["Изготавливаемые ДСЕ"];
        if (!изготавливаемыеДСЕ.Any())
            return;

        Объект ДСЕ = изготавливаемыеДСЕ.FirstOrDefault();
        ReferenceObject DSE = (ReferenceObject)ДСЕ;

        ВыполнитьМакрос(Макрос_Переменные, "ЗаполнитьОсновныеНадписи", (ТехнологическийПроцесс)ТекущийОбъект);

        ВыполнитьМакрос(Макрос_Теги, "ОчиститьЗамечания");

        //Инициализация шаблона
        Текст текст = Текст["Текст1"];
        ШаблонСтроки Раздел_шаблон = new ШаблонСтроки();
        ШаблонСтроки В_шаблон = new ШаблонСтроки();
        ШаблонСтроки Т_шаблон = new ШаблонСтроки();
        ШаблонСтроки Д_шаблон = new ШаблонСтроки();
        ШаблонСтроки С_шаблон = new ШаблонСтроки();
        ШаблонСтроки Рз_шаблон = new ШаблонСтроки();
        ...
        switch (ведомость)
        {
            case "ВО_ф2":
                ...
                С_шаблон = текст["ОбозначениеДСЕ"];
                break;
            case "ВО_ф3":
                Переменная["$graph_33"] = "ВО";
                Раздел_шаблон = текст["Раздел"];
                Т_шаблон = текст["Обозначение ТО"];
                С_шаблон = текст["ОбозначениеДСЕ"];
                Рз_шаблон = текст["Разделитель"];
        ...
        //foreach (ReferenceObject child in DSE.Children.RecursiveLoad())
        //{
        FillNomenclatureObjects((NomenclatureObject)DSE);
        //}
        /*
        if (!_productList.Any())
        	{
            ...
            {
                Строка строкаРаздел = текст.Таблица.ДобавитьСтроку(Раздел_шаблон);
                строкаРаздел["Раздел"].Text = "Изделия";
                
                if (DSE.Class.Name == "Изделие")
                    {
                      Объект дочерний = Объект.CreateInstance(DSE, Context);
                      //cntNom++;
    		          //Получение объекта отчёта
    		          Объекты техпроцессы = дочерний.СвязанныеОбъекты["Техпроцессы"];
    		          if (!техпроцессы.Any())
    			         {
    		             }
    		          else
    		             {
    		              Объект ТП = техпроцессы.FirstOrDefault(t => t["Вариант"].ToString() == "" && 
    		                                       t.Тип.ПорожденОт("Базовый технологический процесс"));
    		              if (ТП == null)
    			             {
    		                 }
    		              else
    		                 {
                                ...
                                int koloper = 0;                                
                                foreach (Операция операция2 in списокОпераций)
                                {            
                                   int количествоСтрок2 = 0;
    	                           string operinfo2 = СводноеНаименование(операция2);
    	                           ...
    	                           int kol2 = 0;
                                    ...
                                    List<ОснащениеОперации> всёОснащение2 = new List<ОснащениеОперации>();
                                    foreach (Оснащение оснащение2 in списокОснащения2)
                                        {
                                            Объект оснащениеСТО2 = оснащение2.СвязанныйОбъект["Оснащение из справочника СТО"];
            
                                            if (оснащениеСТО2 == null)
            	                               continue;
                                            string наим2 = оснащениеСТО2.Параметр["Наименование"];
            
                                            string осн_краткое = наим2.Substring(0, 2);           //Забираем первые две буквы оснащения (ОР; ОШ; ПЧ и ОП и т.д.)
                                            if (осн_краткое == "ОР" || осн_краткое == "ОШ" || осн_краткое == "ПЧ" || осн_краткое == "ОП" || осн_краткое == "ОК" || осн_краткое == "ОУ" || осн_краткое == "ПЦ" 
                                                || осн_краткое == "Ш-" || осн_краткое == "П-" || осн_краткое == "1П" || осн_краткое == "2П" || осн_краткое == "3П" || осн_краткое == "4П" || осн_краткое == "5П" 
                                                || осн_краткое == "6П" || осн_краткое == "7П" || осн_краткое == "8П" || осн_краткое == "9П" || осн_краткое == "КИ" || осн_краткое == "СН" || осн_краткое == "ПФ" 
                                                || осн_краткое == "ми" || осн_краткое == "ри" || осн_краткое == "1Р" || осн_краткое == "2Р" || осн_краткое == "3Р" || осн_краткое == "4Р" || осн_краткое == "5Р"
                                                || осн_краткое == "6Р" || осн_краткое == "7Р" || осн_краткое == "5М")
                                                {
                	                               kol2++;
                	                               continue;
                                                }
                                ...
                                if (koloper > 0)
                                {
                                    Строка строкаИНСТ = текст.Таблица.ДобавитьСтроку(Рз_шаблон);
                                    строкаИНСТ["Разделитель"].Текст = "";
                                    koloper = 0;
                                    cntNom++;
                                    //Заполнение данных ДСЕ
                                    Строка строкаС = текст.Таблица.ДобавитьСтроку(С_шаблон);
                                    строкаС["Номер"].Текст = cntNom.ToString();
                                    строкаС["ОбозначениеДСЕ"].Текст = дочерний["Обозначение"];
                                    строкаС["НаименованиеДСЕ"].Текст = дочерний["Наименование"];
                ...
                else
                {
                	foreach (ReferenceObject child in DSE.Children.RecursiveLoad())
    	               {
    		              Объект дочерний = Объект.CreateInstance(child, Context);
    		              Объект дсе = Объект.CreateInstance(DSE, Context);
    		              string типдочернего = дочерний["Тип"];
    		              string корнаимдсе = дсе["Короткое наименование"];
    		              string корнаимдоч = дочерний["Короткое наименование"];
    	              
    		              if(типдочернего == "Изделие")
    		                  {
    		                  //cntNom++;
    		                  //Получение объекта отчёта
    		                  Объекты техпроцессы = дочерний.СвязанныеОбъекты["Техпроцессы"];
                              ...
                              if (koloper > 0)
                                {
                                    ...
                                    if (корнаимдоч != корнаимдсе)
                                    	{
                                    	   Строка строказаимств = текст.Таблица.ДобавитьСтроку(Т_шаблон);
                                    	   строказаимств["Наименование ТО"].Текст = "см. " + корнаимдоч;
                                    	}
                                    else
                                    {
                                    foreach (Операция операция in списокОпераций)
        ...
        if (!_assemblyList.Any())
        {
        }
        else
        {
            foreach (var nomenclatureObject in _assemblyList)
            {
                Priority(nomenclatureObject);
            }
        }
        _sortassemly1.Sort((x, y) => string.Compare(x.Denotation, y.Denotation));
        _sortassemly2.Sort((x, y) => string.Compare(x.Denotation, y.Denotation));
        _sortassemly3.Sort((x, y) => string.Compare(x.Denotation, y.Denotation));
        _sortassemly4.Sort((x, y) => string.Compare(x.Denotation, y.Denotation));
        _allassemblyListsort = _sortassemly1.Concat(_sortassemly2).Concat(_sortassemly3).Concat(_sortassemly4).ToList();

        if (!_allassemblyListsort.Any())
        {
        }
        else
        {
            Строка строкаРаздел = текст.Таблица.ДобавитьСтроку(Раздел_шаблон);
            строкаРаздел["Раздел"].Text = "Сборочные единицы";

            if (DSE.Class.Name == "Сборочная единица")
            {
                Объект дочерний = Объект.CreateInstance(DSE, Context);
                //cntNom++;
                //Получение объекта отчёта
            ...
            {
                foreach (ReferenceObject child in _allassemblyListsort)
                //foreach (ReferenceObject child in DSE.Children.RecursiveLoad())
                {
                    Объект дочерний = Объект.CreateInstance(child, Context);
                    Объект дсе = Объект.CreateInstance(DSE, Context);
                    string корнаимдсе = дсе["Короткое наименование"];
                    string корнаимдоч = дочерний["Короткое наименование"];
                    string обозндлязаимств = дочерний["Обозначение"];
                    string типдочернего = дочерний["Тип"];
                    if (типдочернего == "Сборочная единица")
                    {
                        //cntNom++;
                        //Получение объекта отчёта
                        Объекты техпроцессы = дочерний.СвязанныеОбъекты["Техпроцессы"];
        ...
        {
            foreach (var nomenclatureObject in _detailList)
            {
                Priority(nomenclatureObject);
            }
        }

        _sortdetail1.Sort((x, y) => string.Compare(x.Denotation, y.Denotation));
        _sortdetail2.Sort((x, y) => string.Compare(x.Denotation, y.Denotation));
        _sortdetail3.Sort((x, y) => string.Compare(x.Denotation, y.Denotation));
        _sortdetail4.Sort((x, y) => string.Compare(x.Denotation, y.Denotation));
        _alldetailListsort = _sortdetail1.Concat(_sortdetail2).Concat(_sortdetail3).Concat(_sortdetail4).ToList();

        if (!_alldetailListsort.Any())
        {
        }
        else
        {
            Строка строкаРаздел = текст.Таблица.ДобавитьСтроку(Раздел_шаблон);
            строкаРаздел["Раздел"].Text = "Детали";

            if (DSE.Class.Name == "Деталь")
            {
                Объект дочерний = Объект.CreateInstance(DSE, Context);
                //cntNom++;
                //Получение объекта отчёта
            ...
            {
                foreach (ReferenceObject child in _alldetailListsort)
                {
                    Объект дочерний = Объект.CreateInstance(child, Context);
                    Объект дсе = Объект.CreateInstance(DSE, Context);
                    string корнаимдсе = дсе["Короткое наименование"];
    ...
    private void FillNomenclatureObjects(NomenclatureObject nomenclatureObject)
    {
        if (nomenclatureObject == null)
            return;

        var nomenclatureType = nomenclatureObject.Class;

        if (nomenclatureType.IsProduct)
        {
            if (!_productList.Contains(nomenclatureObject))
                _productList.Add(nomenclatureObject);
        }
        else if (nomenclatureType.IsAssembly)
        {
            if (!_assemblyList.Contains(nomenclatureObject))
                _assemblyList.Add(nomenclatureObject);
        }
        else if (nomenclatureType.IsDetail)
        {
            if (!_detailList.Contains(nomenclatureObject))
                _detailList.Add(nomenclatureObject);
        }
        else if (nomenclatureType.IsMaterial)
        {
            if (!_materialList.Contains(nomenclatureObject))
                _materialList.Add(nomenclatureObject);
        }
        else if (nomenclatureType.IsInherit(NomenclatureTypes.Keys.Complete))
        {
            if (!_completeList.Contains(nomenclatureObject))
                _completeList.Add(nomenclatureObject);
        }

        var children = nomenclatureObject.Children.OfType<NomenclatureObject>().ToList();
        foreach (var childNomenclatureObject in children)
        {
            FillNomenclatureObjects(childNomenclatureObject);
        }
    }
    private void Priority(NomenclatureObject nomenclatureObject)
    {
        Denotation = nomenclatureObject.Denotation;
        if (Denotation.StartsWith("ИКЛВ"))
        {
            priority = 1;
        }
        else if (Denotation.StartsWith("6Г"))
        {
            priority = 2;
        }
        else if (Denotation.StartsWith("Ф"))
        {
            priority = 3;
        }
        else
        ...
        Sort(nomenclatureObject);
    }

    private void Sort(NomenclatureObject nomenclatureObject)
    {
        var nomenclatureType = nomenclatureObject.Class;

        if (priority == 1)
        {
            if (nomenclatureType.IsAssembly)
            {
                if (!_sortassemly1.Contains(nomenclatureObject))
                    _sortassemly1.Add(nomenclatureObject);
            }
            else if (nomenclatureType.IsDetail)
            {
                if (!_sortdetail1.Contains(nomenclatureObject))
                    _sortdetail1.Add(nomenclatureObject);
            }
        }
        if (priority == 2)
        {
            if (nomenclatureType.IsAssembly)
            {
                if (!_sortassemly2.Contains(nomenclatureObject))
                    _sortassemly2.Add(nomenclatureObject);
            }
            else if (nomenclatureType.IsDetail)
            {
                if (!_sortdetail2.Contains(nomenclatureObject))
                    _sortdetail2.Add(nomenclatureObject);
            }
        }
        if (priority == 3)
        {
            if (nomenclatureType.IsAssembly)
            {
                if (!_sortassemly3.Contains(nomenclatureObject))
                    _sortassemly3.Add(nomenclatureObject);
            }
            else if (nomenclatureType.IsDetail)
            {
                if (!_sortdetail3.Contains(nomenclatureObject))
                    _sortdetail3.Add(nomenclatureObject);
            }
        }
        if (priority == 4)
        {
            if (nomenclatureType.IsAssembly)
            {
                if (!_sortassemly4.Contains(nomenclatureObject))
                    _sortassemly4.Add(nomenclatureObject);
            }
            else if (nomenclatureType.IsDetail)
            {
                if (!_sortdetail4.Contains(nomenclatureObject))
                    _sortdetail4.Add(nomenclatureObject);
            }
        }
    }
    //==========================================================================================================================//
    ...
    */
    ...
    public void Карта(string карта)
    {
        ...
        //инструкцииТП = инструкцииТП.OrderBy(i=>i["Текст инструкции"].ToString());

        foreach /*(var инструкция in инструкцииТП)*/(Инструкция инструкция in инструкцииТП.OrderBy(i => i["Текст инструкции"].ToString()))
        {
            string текстИнструкцииТП = инструкция.Текст;
            текстИнструкцииТП = текстИнструкцииТП + ".";
            ...
            //}
            result = текстИнструкцииТП + ".";
            //Строка строкаИ = текст.Таблица.ДобавитьСтроку(И_шаблон);
            //ВыполнитьМакрос(Макрос_Теги, "ВывестиИнструкции", инструкция, строкаИ);
            if (result != "")
            {
        ...
        foreach (Операция операция in списокОпераций)
        {
            ...
            if (карта == "КТП" || карта == "МОК_ф1" || карта == "МК_ф1" || карта == "МОК_ф2")
            {
                //Заполнение операции
            ...
            if (карта == "КТП" || карта == "МОК_ф1" || карта == "МОК_ф3" || карта == "МОК_ф2")
            {
                //Заполнение информации по переходам
                string неявлпер = ТекущийОбъект.Параметр["Не является переходом"];   //для отмены буквы "О"
                                                                                     //if (неявлпер != "Нет")    //для отмены буквы "О"
                                                                                     //{                       //для отмены буквы "О"
                ...
                foreach (Переход переход in списокПереходов)
                {
                    ...
                    //Материалы на переход
                    if (!запретВводаМатериалов)
                    {
                        Материал[] списокМатериаловПер = переход.Материалы;
                        foreach (Материал материал in списокМатериаловПер)
                        {
                            Строка строкаМ = текст.Таблица.ДобавитьСтроку(М_шаблон);
                            ВыполнитьМакрос(Макрос_Теги, "ЗаполнитьСтрокуМ_дляМК", материал, строкаМ);
                        }
